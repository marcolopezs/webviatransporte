...
class CreativeTable{
    ...
    var $debug;                 // show the errors in the table or not - default: true
    ...
    function init_data_sql(){
        ...
        $result = mysql_query($this->get_sql_select().$this->get_sql_from().' LIMIT 0,1');
        if(!$result)
            $this->sql_error='<b>SQL ERROR:</b> '.mysql_error().'<br/><b>SQL QUERY:</b> '.$this->get_sql_select().$this->get_sql_from().' LIMIT 0,1';
        $row = mysql_fetch_array($result, MYSQL_ASSOC);
        if(is_array($row))
            $this->sql_fields=array_keys($row);
        else
            $flag_no_items=1;
        ...
        if($this->table_key!='' and $this->search=='' and $multiple_search_empty==1 and $sort_empty==1){
            ...
            if(!$result)
                $this->sql_error='<b>SQL ERROR:</b> '.mysql_error().'<br/><b>SQL QUERY:</b> '.$this->get_sql(true);
        }else{
            $result = mysql_query($this->get_sql());
            if(!$result)
                $this->sql_error='<b>SQL ERROR:</b> '.mysql_error().'<br/><b>SQL QUERY:</b> '.$this->get_sql();
        }
        ...
        if($this->total_items<=0){
            $result=mysql_query('SELECT COUNT(*)'.$this->get_sql_from().$this->get_sql_where(false).$this->get_sql_group().$this->get_sql_having());
            if(!$result)
                $this->sql_error='<b>SQL ERROR:</b> '.mysql_error().'<br/><b>SQL QUERY:</b> SELECT COUNT(*)'.$this->get_sql_from().$this->get_sql_where(false).$this->get_sql_group().$this->get_sql_having();

            if($this->get_sql_group()!=''){
                $this->total_items=mysql_num_rows($result);
            }else{
    ...
    function get_sql_expressions($type='select',$pass_clause=false){
        ...
        if(in_array($type,$arr_clauses_cut_keys)){
            ...
            foreach($arr_clauses_cut as $key => $value){
                ...
                if($initial_key!='' and $key_to_cut=='' and stripos($this->sql_query,$value)!==false)
                    $key_to_cut=$key;
            ...
            if(stripos($this->sql_query,$arr_clauses_cut[$initial_key])===false)
                $sql_expression='';
            elseif($key_to_cut=='')
                $sql_expression=($pass_clause ? $arr_clauses_cut[$initial_key] : '').substr($this->sql_query,stripos($this->sql_query,$arr_clauses_cut[$initial_key])+strlen($arr_clauses_cut[$initial_key]));
            else
                $sql_expression=($pass_clause ? $arr_clauses_cut[$initial_key] : '').substr($this->sql_query,stripos($this->sql_query,$arr_clauses_cut[$initial_key])+strlen($arr_clauses_cut[$initial_key]),stripos($this->sql_query,$arr_clauses_cut[$key_to_cut])-(stripos($this->sql_query,$arr_clauses_cut[$initial_key])+strlen($arr_clauses_cut[$initial_key])));
    ...
    function get_sql_select(){
        return $this->get_sql_expressions('select',true);
    }
    ...
    function draw_items_per_page($id_items_per_page=''){
        ...
        if($this->items_per_page_init!==false and $this->total_items>0){
            ...
            $out='<select id="'.$id_items_per_page.'_items_per_page_change" name="'.$id_items_per_page.'_items_per_page_change" onchange="'.$this->change_tags($this->items_per_page_url).'">';
            ...
            for($i=0; $i<count($this->data);$i++){
                ...
                foreach($this->data[$i] as $key => $value){
                    ...
                    if($this->display_cols[$j]!='f'){
                        $out.='<td'.(($i==0 and $this->width!='' and $arr_width[$key]>0) ? ' width="'.$arr_width[$key].'"' : '').$cell_attribute.'>'.str_replace('#ROW#',$i+1,$this->change_tag_col($this->change_tags(($this->format_cols[$j]!='' ? $this->format_cols[$j] : $value)),$this->data[$i])).'</td>';
                    }
                    $j++;
                }
                $out.='</tr>';
            }
        }elseif($this->sql_error!='' and $this->debug!==false){

            $arr_header=explode(',',$this->header);

            $out.='<tr id="'.$this->id.'_error"><td colspan="'.count($arr_header).'">'.$this->sql_error.'</td></tr>';
